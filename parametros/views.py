...
from parameters.serializers import StateSerializer
...
class SubSectoresClasificadorViewSet(SoftDeleteModelViewSet):
    ...
    def get_queryset(self):
        ...
        denominativo_id = self.request.query_params.get('denominativo_id',None)

        if sector_id:
            queryset = queryset.filter(
               denominativo__sector_id=sector_id
            )
        
        if denominativo_id:
            queryset = queryset.filter(
               denominativo__id=denominativo_id
            )
