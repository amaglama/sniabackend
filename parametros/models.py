...
from administracion.models import State
...
class Sector(models.Model):
    ...
    viceministerio_id = models.IntegerField()
    sector_id = models.IntegerField()

    class Meta:
        managed = False
        db_table = 'denominativos_ejecutivos'
        verbose_name_plural = 'Denominativos Ejecutivos'
...
class OrganizacionFinanciera(models.Model):
    ...
    def __str__(self):
        return self.sigla if self.sigla else f'OrganizacionFinanciera {self.id}'
...
class TipoProyecto(BaseCrudModel):
    nombre = models.CharField(max_length=255, unique=True)
    estado = models.CharField(choices=[(status.value, status.value) for status in Estados], default=Estados.HABILITADO.value)
    sub_sector_id = models.IntegerField(blank=True,null=True)
    class Meta:
