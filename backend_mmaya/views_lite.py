...
class TipoProyectoViewSet(SoftDeleteModelViewSet):
    ...
    def get_queryset(self):
        queryset = super().get_queryset()
        sub_sector_id = self.request.query_params.get('sub_sector_id',None)
...
class ProgramasViewSet(SoftDeleteModelViewSet):
    ...
    ordering_fields = ['sigla_prog_convenio','created_at']
...
class EstadoDetalladoViewSet(SoftDeleteModelViewSet):
    ...
    def get_queryset(self):
        ...
        if estado_id:
            queryset = queryset.filter(estado_proyecto=estado_id).order_by('nombre')
        return queryset
